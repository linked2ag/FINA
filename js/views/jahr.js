...
function mrow(label,vals,opt={}){
  ...
  const link=(it&&it.url)?`<a class="linkicon" href="${esc(it.url)}" target="_blank" rel="noopener" title="${t('year.linkTip')}">${LINK_SVG}</a>`:'';
  ...
  const mark=m=>{
    let sym='';
    if(it) sym=it.amounts[m-1]===0?'':(paidAt(it,m)?'<span class="mk-ok">&#10003;</span>':(estOf(it)?'<span class="mk-q">?</span>':''));
    else if(kk) sym=kakDone(kk,m)?'<span class="mk-ok">&#10003;</span>':(state.kak[kk].estimated?'<span class="mk-q">?</span>':'');
    else return '';
...
function viewJahr(){
  ...
  body+=mrow(t('year.kakRow'),MONTHS.map((_,i)=>kakeiboFor(i+1)),{cls:'sec r-flex'});
  kakCats().forEach(k=>{
    if(!state.kak[k]) return;
    const v=MONTHS.map((_,i)=>kakVal(k,i+1));
    if(v.some(x=>x!==0)||state.kak[k].plan.some(x=>x!==0)) body+=mrow(esc(keyLabel(k)),v,{kak:k,cls:'r-flex'});});
  body+=spacer();
  ...
  return `<div class="sechead yearbar" id="yearBar">
      ...
      <span style="display:flex;gap:16px;align-items:center;flex-wrap:wrap">
        ...
        <button class="btn small" id="btnNew">${t('year.addItem')}</button></span></div>
    ...
    ${t('year.hintTerm',MONTHS_LONG[CUR-1])}
