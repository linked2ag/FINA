# FINA Struktur v260905-2

**Stand:** 5. September 2026, zweite Fassung des Tages · Fassung der Anwendung `26.8.30` (`VERSION` in `js/config.js`)

**Vorgängerin:** `FINA Struktur v260905-1.md`. Was sich gegenüber ihr geändert hat, steht unten unter „Änderungen an dieser Fassung".
...
| `impRows{}` | **Nur bei importierten Monaten.** Schlüssel = Monat 1–12, Wert = Liste der Quellzeilen `{d, v, r}`: Datum als Text `"TT.MM.JJ"`, Betrag, und `r[]` = die **drei Referenzen** der Zeile in ihrer Rangfolge (Referenz 1 · 2 · 3; leere Enden fehlen, eine Lücke in der Mitte bleibt als `""`). Aus ihnen ist `amounts[m]` entstanden. **Ältere Zeilen** (bis 5.9.26 vormittags) tragen statt `r` einen Text `x` aus allen übrigen Spalten; gelesen wird beides (`impRowText()` in `js/calc.js`). | `{"3":[{"d":"05.03.26","v":-850,"r":["Hausverwaltung","Miete März"]}]}` |
...
| `r[]` | Die **drei Referenzen** der Zeile in ihrer Rangfolge (Referenz 1 · 2 · 3), wie bei `impRows`. Referenz 1 steht im Reiter „Import Details" an der Stelle der früheren Unterkategorie, 2 und 3 als Notiz darunter (`txSub()` / `txNote()` in `js/calc.js`). | `["Supermarkt","REWE Berlin"]` |
...
| `f{}` | Welche Spalte welches FINA-Feld trägt: `{date, amount, ref1, ref2, ref3}`, je Spaltennummer oder `-1`. **Es gibt nur diese fünf Felder**, für reguläre wie für flexible Posten. Ältere Einträge mit `{main, cat, desc}` übersetzt `migrate()` beim Öffnen: Hauptkategorie → Referenz 1, Unterkategorie → Referenz 2, Beschreibung → Referenz 3, wobei fehlende aufrücken. | `{"date":0,"amount":4,"ref1":3,"ref2":-1,"ref3":-1}` |
...
## Änderungen an dieser Fassung (5.9.26, zweite Fassung)
